(function() {
  var Engine = window.Engine = {
    ...
    STORAGE_KEY: window.ADarkRoomConfig.storageKey,
    LANGUAGE_STORAGE_KEY: window.ADarkRoomConfig.languageStorageKey,
    VERSION: 1.3,
    ...
    loadGame: function() {
      try {
        var serializedState = localStorage.getItem(Engine.STORAGE_KEY);
        var savedState = serializedState ? JSON.parse(serializedState) : null;
        if(savedState && typeof savedState == 'object' && !Array.isArray(savedState)) {
          window.State = savedState;
          $SM.updateOldState();
          Engine.log("loaded save!");
        } else {
          throw new Error('No valid save found.');
        }
      } catch(e) {
        window.State = {};
    ...
    generateExport64: function(){
      var string64 = Base64.encode(localStorage.getItem(Engine.STORAGE_KEY) || '{}');
    ...
    import64: function(string64) {
      ...
      var importedState = JSON.parse(decodedSave);
      if(!importedState || typeof importedState != 'object' || Array.isArray(importedState)) {
        throw new Error('Invalid save data.');
      }
      localStorage.setItem(Engine.STORAGE_KEY, JSON.stringify(importedState));
      location.reload();
    },
          ...
          start: {
            ...
            buttons: {
              'ios': {
                ...
                nextScene: 'end',
                onChoose: function () {
                  Engine.log('External app promotion is disabled.');
                }
              },
              'android': {
                text: _('android'),
          ...
          start: {
            ...
            buttons: {
              'facebook': {
                ...
                nextScene: 'end',
                onChoose: function() {
                  Engine.log('External sharing is disabled.');
                }
              },
              'google': {
                text:_('google+'),
    ...
    saveLanguage: function(){
      ...
      if(lang && typeof Storage != 'undefined' && localStorage) {
        localStorage.setItem(Engine.LANGUAGE_STORAGE_KEY, lang);
      }
    },
