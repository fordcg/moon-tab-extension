...
function attachFloatingAssistantFrame(url: string): { ok: true } | { ok: false; message: string } {
  ...
  // Reject legacy control-beacon URLs; pet companion lives in the side panel only.
  if (isLegacyControlBeaconUrl(url)) {
    return { ok: false, message: "控制信标已移除，请使用侧栏 AI 伴侣" };
  }
...
function closeFloatingAssistantFrame(): void {
  ...
  // Best-effort cleanup for leftover orbs from older builds.
