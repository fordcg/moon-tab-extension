...
import { getSkillModelTools } from "../../skills/loadSkills";
...
const RAW_AVAILABLE_MODEL_TOOLS: Omit<ModelToolRegistryEntry, "toolClassification">[] = [
  ...
  },
  {
    ...
    description:
      "调用 imagefree.net 按 prompt 和 aspect_ratio 生成图片，返回生成后的图片 URL。会激活 imagefree.net 标签页完成 Cloudflare Turnstile；若出现验证框请在该页手动通过后等待工具继续。非官方接口，可能较慢。",
    parameters: {
      type: "object",
      properties: {
        prompt: {
...
export const AVAILABLE_MODEL_TOOLS: ModelToolRegistryEntry[] = [
  ...RAW_AVAILABLE_MODEL_TOOLS.map((tool) => ({
    ...tool,
    toolClassification: getRequiredToolClassification(tool.id),
  })),
  // Skill packages contribute tools without growing this file per skill.
  ...getSkillModelTools(),
];
