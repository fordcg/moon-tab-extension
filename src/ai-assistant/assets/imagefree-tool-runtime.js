import {
  A as getBuiltInTools,
  dt as listChatSessions,
  lt as readAppSetting,
  xt as writeChatSession,
  yt as writeAppSetting,
} from "./tabCapture-CF6ZxIgy.js";

export const IMAGEFREE_TOOL_ID = "imagefree.generate_image";
export const IMAGEFREE_TOOL_NAME = "imagefree_generate_image";
...
export const IMAGEFREE_TOOL_DEFINITION = {
  id: IMAGEFREE_TOOL_ID,
  ...
  parameters: {
    ...
    properties: {
      ...
      aspect_ratio: {
        type: "string",
        enum: Array.from(IMAGEFREE_ASPECT_RATIOS),
...
registerImagefreeTool();
globalThis.__imagefreeGenerateTool = executeImagefreeGenerateTool;
// Keep MV3 service worker startup fast so action/command listeners register immediately.
void migrateImagefreeToolSelection();

export function registerImagefreeTool() {
  try {
    const tools = getBuiltInTools();
    if (!Array.isArray(tools) || tools.some((tool) => tool?.id === IMAGEFREE_TOOL_ID)) {
      return;
    }
    tools.push(IMAGEFREE_TOOL_DEFINITION);
  } catch (_error) {
  }
}

export async function migrateImagefreeToolSelection() {
  try {
    if (!hasImagefreeToolRegistered()) {
      return;
    }
    if (await readAppSetting(IMAGEFREE_TOOL_SELECTION_MIGRATION_KEY)) {
      return;
    }

    const now = Date.now();
    const chatPreferences = await readAppSetting("chatPreferences");
...
function migrateToolSelectionObject(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const enabledToolIds = Array.isArray(value.enabledToolIds) ? value.enabledToolIds : [];
...
function shouldAppendImagefreeTool(enabledToolIds) {
  ...
  return enabledToolIds.some(
    (id) => builtInIds.has(id) && id !== IMAGEFREE_TOOL_ID && !id.startsWith("browser."),
  );
}

function hasImagefreeToolRegistered() {
  return getBuiltInTools().some((tool) => tool?.id === IMAGEFREE_TOOL_ID);
}

export async function executeImagefreeGenerateTool(toolCall, fetcher = globalThis.fetch, options = {}) {
  const validation = normalizeImagefreeArguments(toolCall?.arguments);
  if (!validation.ok) {
    return imagefreeError(toolCall, validation.message);
  }
...
function normalizeImagefreeArguments(input) {
...
async function generateImagefreeImage(input, fetcher, options = {}) {
...
async function resolveImagefreeTurnstileToken(token, options = {}) {
...
function createImagefreeTurnstileTokenProvider(options = {}) {
...
async function requestImagefreeTurnstileTokenFromTab() {
  ...
  if (result?.ok && normalizeTurnstileToken(result.token)) {
    return result.token;
  }
  throw new Error(result?.message || "Imagefree 真人验证未完成。");
}

async function ensureImagefreeTurnstileTab(chromeApi) {
  const tabs = await chromeCall(chromeApi.tabs.query, { url: `${IMAGEFREE_BASE_URL}/*` }).catch(() => []);
  const existing = Array.isArray(tabs)
    ? tabs.find((tab) => typeof tab?.id === "number" && /^https:\/\/imagefree\.net\//i.test(tab.url || ""))
    : undefined;
  if (existing) {
    return existing;
  }
  return chromeCall(chromeApi.tabs.create, { url: IMAGEFREE_PAGE_URL, active: false });
}

async function focusImagefreeTurnstileTab(chromeApi, tab) {
  if (typeof tab?.id !== "number" || !chromeApi?.tabs?.update) {
    return;
  }
  const updated = await chromeCall(chromeApi.tabs.update, tab.id, { active: true }).catch(() => undefined);
...
async function waitForImagefreeTabReady(chromeApi, tabId, timeoutMs) {
...
function imagefreeTurnstileTokenScript(siteKey, timeoutMs) {
  const existing = window.__moonTabImagefreeTurnstileTokenPromise;
  if (existing) {
    return existing;
  }
  ...
  const promise = new Promise((resolve) => {
    ...
    const cleanup = () => {
      ...
      host.remove();
      delete window.__moonTabImagefreeTurnstileTokenPromise;
    };
    const done = (result) => {
      cleanup();
      resolve(result);
    ...
    const tryRender = () => {
      ...
      try {
        ...
        done({ ok: false, message: `Imagefree 真人验证组件加载失败：${error?.message || String(error)}` });
      }
    };
  ...
  window.__moonTabImagefreeTurnstileTokenPromise = promise;
  return promise;
}

function normalizeTurnstileToken(token) {
...
async function pollImagefreeStatus(fetcher, taskId) {
...
async function requestImagefreeJson(fetcher, url, init, options = {}) {
  let response;
  try {
...
function imagefreeError(toolCall, message) {
  return {
    toolCallId: toolCall?.id,
    name: toolCall?.name || IMAGEFREE_TOOL_NAME,
    content: message,
