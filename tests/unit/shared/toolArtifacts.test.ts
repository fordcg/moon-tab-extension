...
describe("通用工具附件聚合", () => {
  ...
  it("通用工具附件在 live collection 聚合后会脱敏字符串化 JSON body", () => {
    const message = createAssistantMessage({
      toolAttachments: [
        {
          id: "generic-stringified-json-sensitive-1",
          kind: "page-context",
          title: "页面上下文",
          summary: JSON.stringify({
            requestBody: JSON.stringify({ api_key: "xai-secret", token: "summary-token" }),
          }),
          details: JSON.stringify({
            requestBody: JSON.stringify({ password: "123456", access_token: "abc" }),
          }),
          sourceToolCallId: "call-generic-stringified-json",
          createdAt: 12,
          redacted: false,
          truncated: false,
        },
  ...
  it("通用工具附件在 live collection 聚合后会脱敏内嵌 JSON snippet", () => {
    const message = createAssistantMessage({
      toolAttachments: [
        {
          id: "generic-embedded-json-sensitive-1",
          kind: "page-context",
          title: "页面上下文",
          ...
          sourceToolCallId: "call-generic-embedded-json",
          createdAt: 13,
          redacted: false,
          truncated: false,
        },
  ...
  it("通用工具附件会脱敏嵌套直接内嵌 JSON snippet 的敏感父键", () => {
    const attachment = normalizeToolAttachment({
      id: "generic-nested-direct-embedded-json-sensitive-1",
      kind: "page-context",
      title: "页面上下文",
      summary: 'payload={"password":{"value":"123456"}}',
      details: 'items={"items":[{"token":"abc"}],"api_key":{"value":"xai-secret"}}',
      createdAt: 15,
      redacted: false,
      truncated: false,
    });
