...
| Phase 5 Stringified JSON Redaction Fix | `bf18315` | 递归脱敏字符串化 JSON body |
...
| `npx vitest run tests/unit/shared/toolArtifacts.test.ts -t "内嵌 JSON\|字符串化 JSON"` | RED 后通过 | 2026-07-07 Phase 5 embedded JSON snippet redaction fix；补丁前失败于 `Request body: {"api_key":"xai-secret"}` 泄露，补丁后 1 file / 2 tests passed / 22 skipped |
...
| `npx vitest run tests/unit/shared/toolArtifacts.test.ts -t "嵌套\|直接内嵌 JSON"` | RED 后通过 | 2026-07-07 Phase 5 nested direct embedded JSON snippet redaction fix；补丁前失败于 `payload={"password":{"value":"123456"}}` 泄露，补丁后 1 file / 2 tests passed / 24 skipped |
