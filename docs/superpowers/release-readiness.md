...
| 打包产物 | `scripts/package-extension.test.ts`、`scripts/verify-release-readiness.mjs` | 发布包包含 manifest 声明页面、background、content、A Dark Room 静态运行时、许可证和 build-info，排除测试与旧产物 |
...
5. 打开新标签页确认 Moon Tab 渲染；从页面导航到游戏，确认“暗室”和“生火”可见。
...
- A Dark Room 仅由扩展自有页面打开，`src/pages/game/*` 不声明为 web-accessible resources。
