...
export const createAiPreviewController = ({ elements, callbacks, deps, config = {} }) => {
  ...
  const searchRequestTimeout = config.searchRequestTimeout ?? 15000;
  const transientRetryDelays = config.transientRetryDelays ?? [450, 1100];
  const bingSearchOriginPattern = config.bingSearchOriginPattern ?? "https://www.bing.com/*";
  const bingRssEndpoint = config.bingRssEndpoint ?? "https://www.bing.com/search?format=rss&mkt=zh-CN&q=";
  ...
  const requestAiWebsiteCandidates = async (originalQuery, refinedQuery, candidates, settings) => {
    ...
    const timeoutId = window.setTimeout(() => controller.abort(), searchRequestTimeout);
  ...
  const buildPreview = (decision, originalQuery) => {
  ...
  const showPreviewFromDecision = (decision, originalQuery) => {
    const preview = buildPreview(decision, originalQuery);
    showPreview(preview);
    return preview;
  };
